...
local NUM_ROWS = 14
local NUM_CHARS = 28
local COLOR = "#DFD700" --"#CCCCCC" -- "#33FF00" -- "#FFB000"
local RADIUS = 6
...
local function update(pos, objref)
	...
	local texture = lcdlib.make_mono_texture(nvm.text or {}, NUM_CHARS, NUM_ROWS, COLOR)
	objref:set_properties({ textures = {texture}, visual_size = {x=0.90, y=0.90} })
	t = core.get_us_time() - t
	print("time =", t, "#texture =", #texture)
end
...
minetest.register_node("techage:ta3_monitor", {
	...
	node_box = {
		...
		fixed = {
			...
			{-16/32, -16/32, -15/32,  16/32, 16/32, 15/32},
		},
	},
	selection_box = {
		type = "fixed",
...
techage.register_node({"techage:ta3_monitor"}, {
	...
	on_beduino_receive_cmnd = function(pos, src, topic, payload)
		print("on_beduino_receive_cmnd", topic, payload)
